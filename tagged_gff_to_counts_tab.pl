...
if (@ARGV == 0 ) {
    print STDERR "USAGE: tagged_gff_to_counts_tab.pl -g tagged_gff [-c gene_feature_name] [-f tag_feature_name] -s sequence_file [-o output_file] [-N library_size]\n";
    exit;
}
...
#	my @addcountsarr =$$gene->get_tag_values('SLaddcounts');
	...
	print $outputfh "\t",join(",",@addcountsarr), "\t", join(",",@sladds),"\t", join(",",@fputrarr),"\t",$major_fputr,"\t",$major_fputr_count,"\t",$shortest_fputr,"\t",$longest_fputr;
    } else {
	print $outputfh "\t.\t.\t.\t.\t.\t.\t.";
    }
