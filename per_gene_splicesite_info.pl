...
my $UTR_length_cutoff = 0; # 0 == no filtering

if (@ARGV == 0 ) {
    print STDERR "USAGE: per_gene_splicesite_info.pl -s ref_sequence_file -g reference_gff [-c gene_feature_name] -t tag_map_gff [-f tag_feature_name] [-o output_file] [-U 5pUTR_length_cutoff, 0==any ok]\n";
    exit;
}
...
for (my $i = 0; $i < $ngenes ; $i++) {
	...
	while ( $keep_checking_tags ) {
		...
		if($$current_tag_map->end >= $$current_gene->end) {
		    ...
		    if ( $UTR_length_cutoff == 0 || ($UTR_length_cutoff > 0 && $$current_tag_map->end - $$current_gene->end + 1 <= $UTR_length_cutoff ) ) {
	...
	while( $keep_checking_tags ) {
		...
		if($$current_tag_map->start <= $$current_gene->start) {
		    # distance filter?
		    if ( $UTR_length_cutoff == 0 || ($UTR_length_cutoff > 0 && $$current_gene->start - $$current_tag_map->start + 1 <= $UTR_length_cutoff) ) {
...
if ($UTR_length_cutoff != 0) {
    print "A UTR length cutoff of $UTR_length_cutoff was used. $tag_maps_mapped_intergenic mapped intergenically.\n";
} else {
    print "No UTR length cutoff was used. This may help with a few natural large 5pUTRs, but beware of huge UTRs and possibly noisy expression levels due to missing annotations.\n";
}
