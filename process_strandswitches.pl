...
#    if ($tu->primary ne "mRNA") {
#	$lasttu = $tu;
#	next;
#    }

    if (!defined $lasttu) {
	# first tu encountered!
	$lasttu=$tu;
	next;
    }

    if (defined($lasttu) && ($tu->primary ne $lasttu->primary) ) {
	# transition between different kinds of units.. mark differently?		
    }

    if ( $lasttu->strand eq $tu->strand ) {
	# same strand transition, likely between different types..
	

    }
    
    if ( ($lasttu->strand == -1) && ($tu->strand == 1) ) {
	# head head switch
    }
	...
	# take last N genes of lasttu

	(@alllasttugenes) = ($lasttu->get_tag_values('gene_name'));
	$n = scalar(@alllasttugenes);

	my @lasttugenes;
	if ( $n >= $pickN ) {
	    (@lasttugenes)= (@alllasttugenes[($n-$pickN)..($n-1)]);
	} else {
	    @lasttugenes = @alllasttugenes;
	}
	# take first N genes of tu
	

	
	my ($ref_name) = ($tu->get_tag_values('ref_name'));
	my $switchstart = $lasttu->end;
	my $switchend = $tu->start;

	print "$ref_name\tproceSS\tTTS\t$switchstart\t$switchend\n";
